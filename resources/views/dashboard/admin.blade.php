...
<div class="d-flex flex-column flex-sm-row justify-content-between align-items-start align-items-sm-center gap-3 mb-4">
    ...
    <div class="d-flex flex-wrap gap-2 w-100 w-sm-auto">
        @if(Auth::user()->canManageSales())
            <a href="{{ route('sales.create') }}" class="btn btn-gold px-3 flex-grow-1 flex-sm-grow-0 text-center">
                <i class="bi bi-cart-plus me-1"></i> Transaksi Baru
            </a>
        @endif
        @if(Auth::user()->canManageCars())
            <a href="{{ route('cars.create') }}" class="btn btn-outline-gold px-3 flex-grow-1 flex-sm-grow-0 text-center">
        ...
        <div class="card card-stat p-3 bg-white h-100">
            <div class="d-flex align-items-center justify-content-between">
                <div>
                    ...
                    <small class="text-muted d-none d-sm-inline">Unit terdata di showroom</small>
                </div>
                <div class="stat-icon">
        ...
        <div class="card card-stat p-3 bg-white h-100" style="border-left-color: #198754;">
            <div class="d-flex align-items-center justify-content-between">
                <div>
                    ...
                    <small class="text-muted d-none d-sm-inline">Siap untuk dijual</small>
                </div>
        ...
        <div class="card card-stat p-3 bg-white h-100" style="border-left-color: #ffc107;">
            <div class="d-flex align-items-center justify-content-between">
                <div>
                    ...
                    <small class="text-muted d-none d-sm-inline">Dalam proses/booking</small>
                </div>
        ...
        <div class="card card-stat p-3 bg-white h-100" style="border-left-color: #0d6efd;">
            <div class="d-flex align-items-center justify-content-between">
                <div>
                    ...
                    <small class="text-muted d-none d-sm-inline">Transaksi selesai</small>
                </div>
...
<div class="row g-4 mb-4">
    ...
    <div class="col-lg-4">
        <div class="card border-0 shadow-sm h-100">
            ...
            <div class="card-body d-flex flex-column justify-content-center text-center p-3 p-md-4">
                <div class="p-3 bg-light rounded-3 mb-3 border">
                    <div class="text-muted small text-uppercase mb-1 fw-bold">Akumulasi Penjualan</div>
                    <h3 class="fw-extrabold text-warning mb-0 fs-4 fs-sm-3">Rp {{ number_format($totalSalesRevenue, 0, ',', '.') }}</h3>
                </div>
                        ...
                        <tbody>
                            @forelse($recentCars as $car)
                                <tr>
                                    ...
                                    <td class="fw-semibold text-warning text-nowrap">{{ $car->formatted_price }}</td>
                            ...
                            @forelse($recentSales as $sale)
                                <tr>
                                    <td class="text-nowrap">
                                        ...
                                        <small class="text-muted font-monospace">{{ $sale->invoice_number }}</small>
                                    </td>
                                    <td>{{ $sale->car->brand ?? '-' }} {{ $sale->car->model_type ?? '' }}</td>
                                    <td>{{ $sale->customer->name ?? '-' }}</td>
                                    <td class="fw-bold text-dark text-nowrap">{{ $sale->formatted_price }}</td>
                                </tr>
                            @empty
                                <tr>
